...
// display all surveys in home page when signed in
    ...
    if(userId){
        Survey.find({ 'userid': userId },(err, surveyList) => {
            if(err){
                ...
                console.log("surveyList",surveyList)
                res.render('index', {title: 'All Surveys',path: '/allsurvey',SurveyList:surveyList,username: req.user? req.user.username : ''});
            }
        })
    }
    else{
